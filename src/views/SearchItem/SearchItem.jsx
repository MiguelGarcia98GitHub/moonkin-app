...
export const SearchItem = () => {
	...
	useEffect(() => {
		...
		async function load() {
			const res = await (
				await fetch(
					`https://us.api.blizzard.com/data/wow/search/item?namespace=static-us&name.en_US=${inputTextValue}&orderby=id&_page=1&access_token=USHC6eLcJyZvrVnvN5LH7sYAtifVPBenhx`
				)
			).json();
			if (!active) {
				return;
